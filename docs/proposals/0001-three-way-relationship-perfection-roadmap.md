...
### Scope decision points(phase 1 内 user 决断)
- Path A frontend swap(full Rust PTY 退役)vs additive sidecar(并存)
- Auth wire(A3 bearer)vs M3 no-auth(localhost 简单)
- 用 react-terminal 还是 AiQ 既有 xterm.js stack
...
3. **AiQ e2e**(若 P1 done):terminal session create + send_text + read_output 端到端
...
1. **Roadmap 整体方向**接受?或要 deprioritize 某 phase?
2. **Phase 1 path 偏好**:additive(Rust PTY + sidecar coexist)OR full swap(Rust PTY 退役)— OR phase 1 plan 阶段再决断?
3. **Phase 2 contract suite granularity**:per-public-API 还是 per-package?— 可 phase 2 内决断
4. **Phase 4 publish 倾向**:倾向 publish 还是 default re-defer?(影响 ADR 0007 scope)
5. **Total calendar 4-5 weeks 接受**否?需 compress / extend?
