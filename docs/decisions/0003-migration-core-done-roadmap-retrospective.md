...
| **C3** | renderer attach via server-node MCP channel(future external IDE / standalone CLI 嵌入铺路) | shipped pragmatic interpretation | **DONE** ContinuoTerminal commit `ca311c0` 2026-05-23。**Pragmatic scope**(per ADR 0006 §3 invariant 不做 wholesale Continuo replacement):ship real-HTTP `createHttpMCPClientAdapter` in `@continuo-terminal/react-terminal`(library deliverable for external embedders;Continuo untouched)。Fetch-based,Bearer token compat with A3,SSE+JSON dual Accept per SDK Streamable HTTP。7 new BDD cases;react-terminal 12/12 PASS;typecheck 0。Continuo `useTerminal.ts` migration remains intentionally NOT done(Continuo keeps Electron IPC path per ADR 0006 §3) |
| **C4** | `terminal-window-isolation` 机制 evaluate — Continuo internal vs server-node abstraction | nice-to-have | **DONE** 2026-05-24 design review verdict **KEEP-CONTINUO-INTERNAL**。`windowId` Electron-specific(Tauri/VS Code 用不同 identifier scheme);A1-A3 `AuthContext.subject` + `authorizeToolCall` 已 provides generic ownership equivalent;no current ContinuoTerminal consumer(M5/A4/CT-B2/X)需 multi-tenant session filter。Continuo `terminal-window-isolation`(`ownerWindowId`/`window-workspace-roots`/`window-seq`,~640 LOC tested)stays product-coupled internal feature。Future-trigger:concrete non-Electron multi-tenant demand → add generic `SessionFilter` + `metadata.ownerId` to `SessionManager`(~50 LOC est)。See `.claude/dev-loop/39-ct-c4-window-isolation-design-review/design-review.md` |
...
5. ~~C4(window-isolation server-node abstraction)— design review only~~ ✓ DONE 2026-05-24 (verdict KEEP-CONTINUO-INTERNAL)
